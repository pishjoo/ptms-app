...
/**
 * Rule that evaluates allocation expiry conditions for a trade case.
 *
 ...
 * - AND allocation data exists (hasAllocation is true)
...
export class AllocationExpiryRule implements TradeCaseRule {
   ...
   */
  public readonly description = 'Monitors allocation expiry conditions for active trade cases with remaining obligations.';
  ...
  public evaluate(snapshot: TradeCaseStateSnapshot): TradeCaseRuleEvaluationResult {
    ...
    // Check if there are remaining obligations beyond allocation
    ...
    if (!hasRemainingObligations) {
      return {
        ...
        ruleName: this.name,
        passed: true,
        message: 'No remaining obligations beyond allocation. No expiration alert required.',
        priority: BusinessPriority.LOW,
      };
    }
    ...
    return {
      ...
      ruleName: this.name,
      passed: false,
      message: `Allocation expiry alert: trade case is ${snapshot.activityStatus} with remaining obligations (Origin: ${snapshot.remainingOrigin}, Declaration: ${snapshot.remainingDeclaration}, Commitment: ${snapshot.remainingCommitment}).`,
      priority,
    };
  }
